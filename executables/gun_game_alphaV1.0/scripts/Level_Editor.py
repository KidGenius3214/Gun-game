...
class Level_Editor:
    def __init__(self,game):
        ...
        self.map = {"plants":[],"tiles":[]}
        image = self.img_m.load("data/images/16_18.png",(255,255,255))
        for i in range(round(image.get_width()/self.game.TILESIZE)):
            tile_count = i+1
            self.zone1_tiles[str(tile_count)] = get_image(image,i*self.game.TILESIZE,0,self.game.TILESIZE,18,1)
        ...
        for i in range(round(image.get_width()/self.game.TILESIZE)):
            tile_count = i+11
            if i not in [6,7]:
                self.zone1_tiles[str(tile_count)] = get_image(image,i*self.game.TILESIZE,0,self.game.TILESIZE,16,1)
            elif i == 6:
                self.zone1_tiles[str(tile_count)] = get_image(image,i*self.game.TILESIZE,0,19,16,1)
            elif i == 7:
                self.zone1_tiles[str(tile_count)] = get_image(image,115,0,self.game.TILESIZE,16,1)
                
        self.buttons = {}
        button_col = 0
        button_row = 0
        for tile in self.zone1_tiles:
            image = self.zone1_tiles[tile]
            tile_id = tile
            self.buttons[tile] = [image,tile_id,pygame.Rect((button_col*18)+440,button_row*20+70,image.get_width(),image.get_height())]
            button_col += 1
            if button_col > 6:
                button_row += 1
                button_col = 0
            ...
            for i, row in enumerate(game_map[layer]):
                for j, tile in enumerate(row):
                    if tile in self.zone1_tiles and tile not in ['1','2','3']:
                        self.game.display.blit(self.zone1_tiles[tile], (j*self.game.TILESIZE-self.scroll[0],i*self.game.TILESIZE-self.scroll[1]))
